...
function _confirm_workbook_import(frm, proceed) {
    ...
    frappe.call({
        method: 'hr_suite.hr_suite.doctype.saudi_monthly_payroll.saudi_monthly_payroll.validate_payroll_workbook',
        args: {
...
function _fetch_employees(frm) {
    ...
    const do_fetch = function() {
        frappe.call({
            method: 'hr_suite.hr_suite.doctype.saudi_monthly_payroll.saudi_monthly_payroll.fetch_employees',
...
function _import_workbook(frm) {
    ...
    const do_import = function() {
        const autoCreateEnabled = !!frm.doc.auto_create_missing_employees;
        frappe.call({
            method: 'hr_suite.hr_suite.doctype.saudi_monthly_payroll.saudi_monthly_payroll.import_payroll_workbook',
            args: {
                doc_name: frm.doc.name,
                file_url: frm.doc.source_workbook
            },
...
function _analyze_workbook(frm) {
    ...
    const do_analyze = function() {
        frappe.call({
            method: 'hr_suite.hr_suite.doctype.saudi_monthly_payroll.saudi_monthly_payroll.preview_payroll_workbook_import',
            args: {
                doc_name: frm.doc.name,
                file_url: frm.doc.source_workbook
            },
...
function _download_gap_report(frm) {
    ...
    const do_download = function() {
        frappe.call({
            method: 'hr_suite.hr_suite.doctype.saudi_monthly_payroll.saudi_monthly_payroll.download_payroll_workbook_gap_report',
            args: {
                doc_name: frm.doc.name,
                file_url: frm.doc.source_workbook
            },
...
function _download_payroll_import_template(frm) {
    ...
    const do_download = function() {
        frappe.call({
            method: 'hr_suite.hr_suite.doctype.saudi_monthly_payroll.saudi_monthly_payroll.download_payroll_import_template',
            args: {
                doc_name: frm.doc.name
            },
            freeze: true,
...
function _download_simple_payroll_import_template(frm) {
    ...
    const do_download = function() {
        frappe.call({
            method: 'hr_suite.hr_suite.doctype.saudi_monthly_payroll.saudi_monthly_payroll.download_simple_payroll_import_template',
            args: {
                doc_name: frm.doc.name
            },
            freeze: true,
...
function _download_employee_setup_template(frm) {
    ...
    const do_download = function() {
        frappe.call({
            method: 'hr_suite.hr_suite.doctype.saudi_monthly_payroll.saudi_monthly_payroll.download_employee_setup_template',
            args: {
                doc_name: frm.doc.name,
                file_url: frm.doc.source_workbook
            },
...
function _autofill_employee_setup_names(frm) {
    ...
    const do_autofill = function(file_url) {
        frappe.call({
            method: 'hr_suite.hr_suite.doctype.saudi_monthly_payroll.saudi_monthly_payroll.autofill_employee_setup_workbook_names',
            args: {
                doc_name: frm.doc.name,
                file_url: file_url
            },
...
function _import_employee_setup(frm) {
    ...
    const do_import = function(file_url) {
        frappe.call({
            method: 'hr_suite.hr_suite.doctype.saudi_monthly_payroll.saudi_monthly_payroll.import_employee_setup_workbook',
            args: {
                doc_name: frm.doc.name,
                file_url: file_url
            },
...
function _create_basic_employees(frm) {
    ...
    const dialog = new frappe.ui.Dialog({
        ...
        primary_action(values) {
            frappe.call({
                method: 'hr_suite.hr_suite.doctype.saudi_monthly_payroll.saudi_monthly_payroll.create_basic_employees_from_payroll',
                args: {
                    doc_name: frm.doc.name,
...
function _create_journal_entry(frm) {
    frappe.confirm(
        ...
        function() {
            frappe.call({
                method: 'hr_suite.hr_suite.doctype.saudi_monthly_payroll.saudi_monthly_payroll.create_journal_entry_from_payroll',
                args: { doc_name: frm.doc.name },
                freeze: true,
...
function _delete_draft_payroll(frm) {
    frappe.confirm(
        ...
        function() {
            frappe.call({
                method: 'hr_suite.hr_suite.doctype.saudi_monthly_payroll.saudi_monthly_payroll.delete_draft_payroll',
                args: { doc_name: frm.doc.name },
                freeze: true,
